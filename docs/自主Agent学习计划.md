...
- 接入 Channel 时必须分别定义 Access、Conversation、Delivery、Reply route 四种 identity。凭证不作 identity；进程重启不新建对话，更换服务账号不复用旧 Stream；运行中输入必须可成为 Interrupt。见 [Channel 接入契约](架构/Channel接入契约.md)。
