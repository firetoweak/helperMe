...
- `ContextRequest`：描述当前 AgentStep 投影输入，包括 Conversation 消息和运行时指令。
- `ModelContext`：仅供当前一次模型请求使用的快照。
- `ContextManager`：唯一的上下文组装入口，不拥有 Session、Plan 或 system prompt。

运行时计划只进入 ModelContext，不写回 Conversation，因此不会污染后续 Turn。

### 生命周期

```text
Session：长期聊天容器
└─ Conversation：跨 Turn 保留
   └─ Turn：一次用户输入的执行
      └─ AgentStep：一次模型响应或工具循环
         └─ Retry：同一次模型请求的瞬时重试
```

由此确定：

- 每个 AgentStep 重新生成 ModelContext。
- 同一个 AgentStep 的所有 Retry 复用同一快照。
- Retry 期间不重新读取 Conversation、不 replan、不重新投影。
- 下一 AgentStep 才能读取工具结果和最新 Plan 状态。
...
Composition Root
    ↓
TurnRuntime
    ├─ RuntimeMode：提供当前 Turn 指令
    └─ ContextManager：构建当前 AgentStep 快照
```
...
- 每个 AgentStep 获取最新指令并重新投影。
- 同一 AgentStep 的 Retry 复用同一个 messages 对象。
- 上下文超限不作为瞬时错误重试。
- PlainMode、PlanningMode、interrupt、Session 链路保持正常。

这一步最重要的认知是：

> Conversation 是事实，RuntimeMode 提供控制状态，ModelContext 是二者在某个 AgentStep 上的临时投影。