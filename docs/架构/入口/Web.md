...
成功的步不挂状态徽标："完成"每步都一样，不带信息，那个位置让给"从这一步之后重开"的控件。失败、中断、待授权仍然挂。
...
源 Session 不需要停止：已提交的历史前缀本就不可变，它可以继续独立运行。

**改写与新分支是两个动作。** 侧栏列的是会话线，不是 journal：改写把新身份接在原来那条线上、顶掉来源身份，线上只剩最后一个身份被列出；作为新分支则谁都不顶，新 journal 自己成一条线。被顶掉的身份照常可读，只是不再代表这条线。这条归属关系是 Host 元数据，和 `auto_authorize`、`paused` 同类，不进 Journal——它是呈现问题，模型不必知道。存在客户端的那份链只是同一个事实的第二个源，换台机器就丢。

分支只改历史，不动文件；分支点之后真有文件改动时由人选退不退，见[工作区版本 · 触发](../运行/工作区版本.md#触发)。
