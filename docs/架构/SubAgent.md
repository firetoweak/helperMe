...
**上下文隔离是主要理由，并行是次要理由。** 整个 `helperme/assistant/context/` 都在一条 Session 内部跟上下文膨胀作战——预算、保护窗、体积外置、Level 1 脱水，见 [上下文](上下文.md)。摘要采用独立的 [Compact](Compact.md) 后台换会话机制。SubAgent 从另一个方向解同一个问题：让中间过程根本不发生在父的 Journal 里。
...
**「一次性」是投递幂等的性质，不是生命周期的性质。** 子 Session 不会被关闭，正常交回结论后停在 `WAITING(user_message)`；中断回传不会改写未知 Attempt，是否已回收由 `subagent.return` 判断。真正保证「最多回收一次」的是回收事实的 `delivery_id = f"{child_session_id}:report"`：同一个子第二次回收会被 Journal 的投递幂等吞掉。代价是失败的子即使又被推进并再次静止，第二条终局也不会被看见。
...
父 Step: reclaim(child_session_id)
  ...
  → 同一条 subagent.report 进父 Journal（cancelled 时不 requests_decision）
  → 待回收集合少一个
```
...
Host 停子是 Worker 回收，不改变 Session 状态。子是只读的，进行中的读取没有外部副作用要补偿。与子自己 `report` 的竞态交给投递幂等：同一个 `delivery_id` 只接纳一次，先写入的终局胜出。已有 `subagent.return` 的子 Worker 不再被唤醒。
...
所以父在决策时从自己已冻结的事实里投影出待回收集合（`project_pending` = 已委派 − 已回收），非空就往 System Prompt 追加一句约束。这个约束只说结论还没齐、不要把已回来的部分当成全部，以及不再需要时收回它；不说「还差几个」，也不再要求必须等齐才能作答。带上计数会让 System Prompt 每收到一条结论就变一次，整段 prefix 缓存跟着失效。
