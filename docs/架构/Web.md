...
界面有 Session 侧栏、当前会话时间线、输入框和停止按钮。时间线同时展示消息和
工具调用卡片。页面级 SSE 与当前选中的 Session 解耦：切换只改变 Host owner 的
...
前端采用 React、TypeScript、Vite、Redux Toolkit、RTK Query、React Router 和
Zod。RTK Query 保存服务端投影；独立 runtime slice 按 Session 保存 activity、
...
- `conversation(session_id)` 投影统一时间线 `items`：`UserMessageReceived` 为
  用户消息；带正文的 `StepCommitted` 为助手消息；同一步的非 `deliver` Command
  为工具卡。未传入 Worker `SessionView` 时，从同一份 Journal 重放得到只读视图。
- 助手消息使用 `StepCommitted` Event ID 作为 `message_id`，并沿用
  `step.trigger_event_id` 作为独立 `output_id`；preview 不进入历史投影。
- 工具卡使用 `command_id` 作为身份。终态来自 `CommandOutcomeReceived`：成功、
  失败（含错误信息）或尚无结果时为运行中。`deliver` 不进入时间线。
...
前端用 `command_id` 合并实时事件与 Journal 卡片：Journal 终态优先；Journal 仍
为运行中时采用更新的实时状态。切换 Session 只改当前视图，不停止其他 Session
上的工具。

本切片不包含 Markdown、授权卡片、SubAgent 展示、Session 管理或历史分叉。
