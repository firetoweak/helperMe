...
不写 Journal，也不假装成 `deliver`。待确认的控制提案来自 Journal 投影
...
Command Authorization 与 Control Approval 都已落地，两者变化原因不同，不抽象成通用审批框架。控制提案的事实形状见[入口与授权 · 控制提案](入口与授权.md#控制提案)；Command 授权见 [Command 授权](Command授权.md)。
