...
正在执行的 Command 被终止，得到「已中断、结果未知」的终局；中断不降级成失败，也不自动重试或自动回滚，见[打断](打断.md)。取消不修改历史，已经发生的外部副作用不因此撤销。之后独立到达的用户消息仍可触发新 Decision，并看到已经入账的结果。普通运行中的输入走下述后到消息规则，不隐式触发取消。
...
不想等旧操作的人用显式打断，那条路会给在执行的 Command 一个中断终局，规则一因此照常成立，见[打断](打断.md)。
