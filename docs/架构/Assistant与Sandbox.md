...
Assistant 把产品接到 Runtime：调用模型、装配工具、投影上下文并编排判定。模型窄接口位于 `helperme/llm/`；执行环境边界位于 `helperme/sandbox/`；工具契约与执行器位于 `helperme/tools/`；MCP 领域位于 `helperme/mcp/`。控制台输入属于 TUI Channel。
...
Assistant 内部相信 Runtime、Dispatcher 与 Tool Binding 的代码契约。`SessionScheduler` 不吞掉 Dispatcher 的未预期异常；TUI 也不能将其打印后继续。异常发生前已经持久化且没有 Outcome 的 Attempt 保持 `unknown`；`/resume` 不自动重试或协调，当前调用链不得把内部 bug 伪装成正常恢复流程。
...
| `helperme/assistant/attachments.py` | 用户与工具图片的 Session 外置引用 |
| `helperme/assistant/delivery.py` | 将模型正文转换为可靠投递 Command，并装配 `deliver` Binding |
| `helperme/assistant/runner.py` | 单 Session 的 Event wake 与 single-flight |
| `helperme/assistant/sessions.py` | 给定 identity 后的 Session 应用操作与 Channel View |
| `helperme/assistant/host/` | Host 进程生命周期与跨 Session 投递 |
| `helperme/assistant/assembly.py` | Assistant 能力装配 |
| `helperme/assistant/compact/` | 后台 handoff 与同 Session 窗口发布 |
| `helperme/assistant/loop_guard.py` | 进展审视提醒，随 Step 提交 |
| `helperme/assistant/builtin_tools.py` | 当前 Sandbox/环境与内置工具装配 |
| `helperme/assistant/mcp.py` | MCP Toolset 到 Assistant Toolset 端口的翻译 |
| `helperme/assistant/control.py` | 对话控制提案的提交后执行、Journal 投影与 Application 审批分派 |
...
| `helperme/assistant/subagent/` | 委派、回收与只读边界 |
...
| `helperme/channels/tui/console.py` | TUI 循环、并发输入与有序 `UserMessageReceived` 映射 |
...
每条 `UserMessageReceived` 都可以成为后续 DecisionMaker 的 trigger。模型调用期间到达的新消息不进入当前冻结 Context，也不取消当前 Step。签发冻结早于该消息的已派发 Attempt 终态前，后到消息还不是可执行决策；旧 Outcome 组不再单独要 Step。Scheduler 按归约后的 Journal 顺序继续推进，见 [Runtime 的“用户输入与后到消息”](Runtime.md#用户输入与后到消息)。
