...
每条 `UserMessageReceived` 都可以成为后续 DecisionMaker 的 trigger。模型调用期间到达的新消息不进入当前冻结 Context，也不取消当前 Step。签发冻结早于该消息的已派发 Attempt 终态前，后到消息还不是可执行决策；旧 Outcome 组不再单独要 Step。Scheduler 按归约后的 Journal 顺序继续推进，见 [Runtime 状态推进模型](Runtime状态推进模型.md) 第 6 节。
