...
- `load_toolset` 会修改当前 Run 的 `ToolsetLoadingState`，但不持有持久状态；
- 作用域属于工具的稳定契约，由 description/runtime instruction 说明为 Run-scoped；
- 执行结果返回本次实际发现的完整工具列表；
...
- 同一 Run 重复加载不重新 discovery，基于首次冻结的 ToolSpec 返回相同 receipt。

### 当前与历史可执行性

- 历史 receipt 只证明过去观察到某项能力，不证明当前 Run 已注册该工具；
- 实际调用只能使用当前轮 `tools` 中暴露的精确名称；
- 当前 Run 未加载时，由模型根据语义显式调用 `load_toolset`；
- `TOOL_NOT_FOUND` 只报告当前 Run 不存在该工具，不猜所属 Toolset、不自动改名、不自动加载，也不推断安装状态。
...
- `load_toolset` 返回完整的精确工具名与描述，并在同一 Run 重复加载时复用冻结快照；
- Toolset 运行时指令明确区分历史发现事实与当前 Run 可执行状态；
- `TOOL_NOT_FOUND` 只报告当前 Run 的机械事实并提示显式加载，不猜测来源；
...
- 相邻 Run 自动化契约测试验证 receipt 留在 Conversation、具体 ToolSpec 不跨 Run 泄漏、下一 Run 重新加载后才能调用。
