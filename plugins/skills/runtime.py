...
class SkillRuntimeCapability:
    """把 enabled Skill 投影为当前 Turn 的两个普通工具。"""
        ...
        async def load_skill(input_data: LoadSkillInput) -> dict[str, Any]:
            ...
            if captured is None:
                return _error_result(SkillRuntimeError(
                    "SKILL_NOT_FOUND",
                    f"Skill {input_data.skill_id} 不在当前 Turn 目录中",
    ...
    async def _require_current_record(
        ...
        if (
            ...
            raise SkillRuntimeError(
                "SKILL_CATALOG_STALE",
                f"Skill {captured.name} 已在当前 Turn 创建后变化",
                hint="在下一个 Turn 使用最新 Skill 目录重新选择。",
                data={
