...
class CommandEnvironmentPolicyTest(unittest.TestCase):
    def test_only_forwards_baseline_and_explicit_names(self):
    ...
    def test_environment_name_matching_is_case_insensitive(self):
...
class PowerShellCommandRunnerTest(unittest.TestCase):
    def test_preserves_stdout_stderr_and_explicit_exit_code(self):
    ...
    def test_uses_powershell_composition_semantics(self):
        runner = PowerShellCommandRunner()
    ...
    def test_does_not_inherit_unlisted_host_variable(self):
        runner = PowerShellCommandRunner()
    ...
    def test_timeout_returns_partial_result_and_stops_waiting(self):
        runner = PowerShellCommandRunner()
    ...
    def test_missing_powershell_is_an_expected_boundary_error(self):
        runner = PowerShellCommandRunner(
            executable="missing-powershell-for-helperme-test.exe"
        )

        with self.assertRaisesRegex(FileNotFoundError, "未找到 PowerShell"):
            runner.run("Write-Output ok", Path.cwd(), 10)


@unittest.skipUnless(POWERSHELL, "需要 Windows PowerShell")
class ExecuteCommandToolTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_nonzero_exit_is_a_completed_command(self):
        result = self.execute({
            "root": "project",
    ...
    def test_workspace_effect_defaults_to_may_write(self):
        result = self.execute({
            "root": "project",
    ...
    def test_read_only_workspace_effect_is_returned(self):
        result = self.execute({
            "root": "project",
            "command": "Write-Output ok",
    ...
    def test_runs_in_workspace_relative_cwd(self):
    ...
    def test_rejects_absolute_and_escaping_cwd(self):
        absolute = self.execute({
            "root": "project",
            "cwd": str(self.workspace_root),
            "command": "Write-Output ok",
        })
        escaping = self.execute({
            "root": "project",
            "cwd": "../outside",
            "command": "Write-Output ok",
        })
    ...
    def test_command_may_use_an_absolute_path_outside_workspace(self):
    ...
    def test_empty_command_is_rejected(self):
        result = self.execute({"root": "project", "command": "  "})

        self.assertEqual(result["code"], "EMPTY_COMMAND")

    def test_timeout_is_a_tool_failure_with_partial_output(self):
        result = self.execute({
            "root": "project",
    ...
    def test_timeout_schema_limits_are_exposed_and_validated(self):
